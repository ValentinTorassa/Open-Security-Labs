...
commands:
  ...
  - "curl -v"
  - "curl -w"
challenge: "Elegí un sitio, medí con curl -w el tiempo de DNS, conexión y total usando curl https://example.com. Luego probá resolviendo directamente por IP con curl --resolve example.com:443:<IP> https://example.com (o probando sobre HTTP simple). Explicá en dos líneas qué etapa de tiempo desapareció y por qué."
nextLabs:
...
La terminal imprime los tiempos medidos en segundos:
...
Al consultar directamente por IP con --resolve, el tiempo de DNS se reduce a 0s.
```
