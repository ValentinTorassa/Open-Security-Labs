...
En una respuesta normal de `dig` mirás tres bloques: `ANSWER` (la respuesta), `AUTHORITY` (quién manda en esa zona) y el **TTL**, la segunda columna (a la izquierda de la clase `IN` y del tipo de registro):
...
<Callout type="error">
  ...
  magia ni mala suerte: es el TTL todavía corriendo. `dig` saltea la caché
  interna del navegador y te muestra el TTL decreciente que todavía le queda a
  la caché de tu resolver. Por eso es la herramienta correcta para debuggear.
</Callout>
...
- Salto 3: example.com (Autoritativo) → devuelve registro A: 93.184.216.34
...
Un cambio reciente no se refleja de inmediato porque los resolvers intermedios
retienen la respuesta anterior en memoria hasta que su TTL decrece a 0.
```

<Callout type="evidencia">
  En ciberseguridad y operaciones, el DNS trace es tu evidencia primaria de delegación autoritativa y verificación de integridad frente a desvíos o secuestros de tráfico.
</Callout>
